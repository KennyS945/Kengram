...
const Profile = () => {
  ...
  useEffect(() => {
    if (currentUser && user.id) {
      // Check if logged-in user is in the profile's followers array
      const followers = Array.isArray(currentUser.followers)
        ? currentUser.followers
        : (typeof currentUser.followers === 'string' 
          ? JSON.parse(currentUser.followers || '[]')
          : []);
        ...
        <div className="flex xl:flex-row flex-col max-xl:items-center flex-1 gap-7">
              ...
              <StatBlock 
                value={
                  Array.isArray(currentUser.followers)
                    ...
                    : (typeof currentUser.followers === 'string' 
                      ? JSON.parse(currentUser.followers || '[]').length 
                      : 0)
                } 
                label="Followers" 
              />
              <StatBlock 
                value={
                  Array.isArray(currentUser.following)
                    ...
                    : (typeof currentUser.following === 'string' 
          ...
          <div className="flex justify-center gap-4">
            ...
            <div className={`${user.id === id && "hidden"}`}>
              <Button 
                type="button" 
                className={isFollowing ? "shad-button_ghost px-8" : "shad-button_primary px-8"}
